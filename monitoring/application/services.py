...
class EmfMonitoringApplicationService:
    ...
    def create_reading(self, serial_number, field_ut, plug, created_at, api_key):
        """1) valida identidad  2) clasifica+valida  3) persiste local  4) sincroniza nube."""
        ...
        synced = self.cloud.sync_reading(saved.serial_number, saved.level, saved.message, saved.field_ut)
        print(f"[EDGE] {saved.serial_number} | {saved.field_ut} uT | {saved.level} | "
              f"plug={saved.plug} | synced={synced}")
        return saved, synced
